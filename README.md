...
> **v1.7.44 — ADR-032 + ADR-033 Radix External Sort (validated through 1B end-to-end; 21.3B Phase 6 in progress)** — Phase 5.2 dotnet-trace + iostat established that the Reference rebuild's bottleneck is *write amplification (~3× useful I/O)*, not CPU and not raw bandwidth. The earlier parallel rebuild + sort-insert (ADR-030 Phases 2-3, 1.7.36-37) traded compute for overhead — wall-clock-neutral but with hidden GC pressure (453 s of finalizer time at 100 M) and lock-acquire chains. **Reverted in 1.7.38**, replaced with the right execution: **ADR-032** introduces LSD radix sort + external chunked merge + `AppendSorted` for both GPOS and trigram secondary indexes (sequential B+Tree append eliminates random-leaf write amplification); **ADR-033** applies the same architecture to the bulk-load primary GSPO path. 100 M Reference rebuild dropped from 511 s baseline to **48.64 s** (10.5× faster). 1 B end-to-end (bulk + rebuild) dropped from ~3h57m to **60m36s** (3.92× combined speedup). All three architectural changes (radix sort + external merge + AppendSorted) verify a single hypothesis in three independent code paths: the access pattern is the lever, not parallelism or hardware. **Phase 6 — full Wikidata 21.3B Reference end-to-end on a single M5 Max laptop, BCL-only — is currently running** (past 96 % at hour 64, expected ~65-72h total wall-clock). Limits register (`docs/limits/`) now documents 7 entries with explicit trigger conditions for the next optimization rounds (sorted atom store, bit-packed atom IDs, MPHF, hardware-accelerated XxHash3, prefetch + pipelined batch intern, bulk-load memory pressure, B+Tree mmap remap). 4,205 Mercury + 25 Solid tests green throughout.
> See [CHANGELOG.md](CHANGELOG.md) and [docs/roadmap/production-hardening-1.8.md](docs/roadmap/production-hardening-1.8.md) for the full picture, [docs/validations/](docs/validations/) for the measurement arc, and [docs/limits/](docs/limits/) for the documented next rounds.
