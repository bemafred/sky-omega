...
See **[DRHOOK.md](DRHOOK.md)** for the DrHook debugging workflow (observe-before-fixing), the 22-tool MCP reference, how to run each test kind, and the probe corpus — the runtime-observation counterpart to [MERCURY.md](MERCURY.md). **Before driving DrHook, read its "Lifecycle discipline & common pitfalls" section** — DrHook is the first debugger built for an LLM operator, so there is no training corpus on using one and that section is the training surface: no `Debugger.Break()` crutch (the launch hold-gate arms breakpoints pre-main for `dotnet exec`); end sessions with `drhook_stop`, not `drhook_kill` (kill is the anomaly escape hatch); clean up orphaned targets you can see in `drhook_processes`; single-file (`PublishSingleFile`) deployments debug at the engine level but currently fail through the live MCP with `0x80131C3C`.
