...
| [Property-path grammar gaps](property-path-grammar-gaps.md) | **Triggered** | Already triggered. Parse-only sweep over WDBench paths + c2rpqs found 12 of 1,199 queries (1.0%) failing parse on three shapes: `^(P)*`, `^((A\|B))+`, `(^A/B)`. ADR for grammar completeness recommended; option B (split `ParsePathPrimary` / `ParsePathExpr`) preferred for long-term extensibility. *Resolved in 1.7.47 via the compositional `ApplyPathExprModifiers` refactor (commit `1be7a4d`).* |
...
| [Cognitive profile validation drought](cognitive-profile-validation-drought.md) | **Triggered** | Already triggered. All measured validations from 2026-04-19 onward are Reference profile; most recent Cognitive measurement (1.7.22) predates ADR-031/032/033/034/035/036 + property-path hardening. Mitigation: 1M/10M/100M Cognitive gradient against 1.7.47. |
...
| [Single-threaded spill blocks parser](spill-blocks-parser.md) | **Resolved (gradient validated, 21.3 B pending)** | ADR-037 pipelined spill landed 2026-05-06: 100 M gradient drops parser_blocked from 33 % (95 s) to ~0 % (0.44 ms); end-to-end −25.6 % wall-clock; queue depth steady-state 0; merge phase unchanged. 21.3 B production validation = cycle 9. |
...
| [Intermediate cleanup deferred to run end](intermediate-cleanup-deferred-to-run-end.md) | **Resolved (in-flight, gradient pending)** | Phase-transition cleanup hook landed in `SortedAtomStoreExternalBuilder.MergeAndWrite` finally block (2026-05-06). `MergeCompletedEvent` extended with `ChunksDeleted` + `ChunkBytesReclaimed`. Gradient validation 1M/10M/100M required before fully resolved + 1.7.49 bump. |
...
| [Observability discipline systematic not reactive](observability-discipline-systematic-not-reactive.md) | **Triggered** (cycle 9 drain — third recurrence) | Cycle 6 merge silent → fixed reactively; cycle 8 trigram drain silent → not fixed; cycle 9 GSPO drain silent → in flight now with no progress emission. The discipline rule is correct; its application is reactive, not proactive. Mitigation: cross-reference audit of `bulk-load-flow.md` stages × `IObservabilityListener` event coverage, surface every gap as a discrete entry; pre-flight instrumentation gate as launch-checklist item. |
