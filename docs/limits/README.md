...
| [Single-threaded spill blocks parser](spill-blocks-parser.md) | Latent (Monitoring) | Cycle 6 wall-clock confirms ≥5% cost from sort-pauses, OR future scale pushes chunk size to 4 GB+ (per-spill pause >30 s), OR external benchmark comparison surfaces ingest-pipeline asymmetry. Mitigation: pipelined spill (double-buffer), composable with struct comparer + parallel sort. |
...
| [Intermediate cleanup deferred to run end](intermediate-cleanup-deferred-to-run-end.md) | **Triggered** (cycle 8 came within ~600 GB of min-free-space abort during drain; manually cleaned) | Already triggered. 3.6 TB of post-merge occurrence chunks held until run end instead of merge end; surfaced 2026-05-05 during cycle 8. On smaller hosts (<7 TB free disk during ingest) this would silently abort the run mid-flight. Mitigation: phase-transition cleanup hooks; either delete chunks at end of `MergeAndWrite` or stream incrementally as `ChunkReader.MoveNext` returns false. |
