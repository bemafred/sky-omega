...
| [BZip2 decompression single-threaded](bz2-decompression-single-threaded.md) | Latent | Phase 7c trace identifies decompression as binding, OR multi-core deployment surfaces where I/O is not the bottleneck (cloud nodes), OR a workflow re-loads the same `.bz2` source repeatedly. Current measured 33 MB/s vs ~300 MB/s parallel ceiling; ~50 min wall-clock per full-Wikidata run recoverable. |
| [AtomStore prefix compression](atomstore-prefix-compression.md) | Latent | ADR-034 SortedAtomStore lands AND validates (sorted layout makes prefix compression cheap to implement), AND/OR memory pressure becomes binding on a target workload, OR a vocabulary-phase comparison vs QLever is being prepared for external publication. ~75 GB memory recovery projected at 21.3 B Wikidata. |
