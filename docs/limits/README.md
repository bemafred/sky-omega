...
| [DrHook.Engine detach races queued-callback flush](drhook-clean-detach.md) | Triggered (root cause confirmed; fix = ADR-006 Phase 2 increment 2) | `DebugSession.Dispose`/`Detach` while the target is actively flooding managed events: mscordbi's RC event thread segfaults flushing queued callbacks as `Detach` tears down its shim (probe 07, 2026-05-21). Quiet targets unaffected (probe 05 detached hr=0). Confirmed by killing the target first → clean detach. Fix: quiescence protocol (`Stop` → `SetAllThreadsDebugState(SUSPEND)` → drain `HasQueuedCallbacks` → `Detach`). |
