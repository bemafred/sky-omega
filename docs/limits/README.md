...
| [ExternalSorter FD pool bypass](externalsorter-fd-pool-bypass.md) | **Triggered** (cycle 10 r4 evidence) | Tier 2 substrate close-out task (2026-05-16 plan). Trigram drain code path opens chunk streams via pre-pool factory, bypassing `ChunkStreamPool.Open`. ~8,192 concurrent open trigram chunk streams at cycle 10 r4 trigram drain peak — 17% headroom on launchd-effective 10,240 FD limit. Pool wired in `dddfda3` (cycle 8) is engaged on atom-merge path but not on trigram-drain path. Fix: replace trigram-drain factory call site with pool-routed open. |
| [N-Triples parser per-triple performance](ntriples-parser-per-triple-perf.md) | Latent (truthy r1 evidence) | Wikidata ships truthy as primary release format AND substrate-build wall-clock becomes published metric, OR non-Wikidata workload arrives `.nt`-only with binding wall-clock, OR Tier 2 `dotnet-trace` profile localizes the gap to a fixable structural cause. Measured ~18-23% per-triple gap (`.nt.bz2` 160 K/sec vs `.ttl.bz2` 196 K/sec) at Wikidata scale; cycle 10 r4 (Turtle) vs truthy r1 (N-Triples) on 1.7.x substrate. |
