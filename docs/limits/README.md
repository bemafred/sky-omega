...
| [Property-list shorthand drops non-first-predicate object projections](property-list-shorthand-projection.md) | **Triggered** (2026-05-17, reproducible on 1.7.70 via `mercury_query` MCP) | SELECT against property-list shorthand (`?s p1 ?o1 ; p2 ?o2`) drops `?o2` (and any subsequent predicate's object) from the result projection — both literals and URIs. Row count correct, first-position variables render, FILTER on the missing variable behaves correctly (binding internally present). Surface unconfirmed: bug is either in `Mercury.Mcp` result serialization or downstream of `QueryExecutor` BGP execution. Workaround: rewrite `;` chains as `.`-terminated patterns. Affects every MERCURY.md "Querying Patterns" example. |
