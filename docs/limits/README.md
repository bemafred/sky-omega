...
| [Property-list shorthand drops non-first-predicate object projections](property-list-shorthand-projection.md) | **Triggered** (2026-05-17, reproducible 1.7.70 across all three Mercury surfaces) | SELECT against property-list shorthand (`?s p1 ?o1 ; p2 ?o2`) drops `?o2` from the result — both literals and URIs. **Surface-localized 2026-05-17 to the SPARQL executor**, not any surface layer: MCP, CLI REPL, and HTTP endpoint all return the same dropped binding. **Parser cleared 2026-05-17** — diagnostic test `Select_PropertyListShorthand_ProducesTwoTriplePatternsWithDistinctObjects` passes; parser correctly produces 2 patterns with distinct object variables. Workaround: rewrite `;` chains as `.`-terminated patterns. Remaining investigation targets: `MultiPatternScan` row assembly, `QueryResults` projection. |
