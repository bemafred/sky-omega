...
- [ ] **exception breakpoints — a distinct location axis** (finding 35): model = type filter (±subclasses) × first-chance/unhandled (`CorDebugExceptionCallbackType`) × optional condition on the exception object (reuses probe-24 member resolution). Two probe-gated unknowns before Engineering:
  ...
  - [ ] **probe 27** (now unblocked) — confirm func-eval works at an **Exception** stop (a different controller state than the Breakpoint stop probe 23 validated; the IDL notes func-eval clears+restores the in-flight exception). Settles conditional exception breakpoints (`ex.Message contains "…"`), reusing probe 24 member resolution on the exception object.
