...
- [x] **`BreakpointPolicy` DONE** (probe 28, finding 38): condition + hit-count gates (`HitCountMode.Equals/AtLeast/Multiple`), `LogMessage` action emitted as a structured `LogRecord` via the widened `IDebugEventSink.OnLog` (default interface method), Suspend ∈ {All, None}. `DebugSession.WaitForPolicyStop(policy, timeout)` generalizes `WaitForConditionalStop`. Four configurations validated against ONE target/breakpoint: conditional breakpoint (`v == 3` surfaces), logpoint (68 lines in 2 s, never surfaces), hit-count-gated logpoint (`Equals(3)` — exactly one log line in the window; sampling), fault (`ConditionError` + `IsFault` `LogRecord` — finding 35 tri-state). 37 unit tests pass (15 new pinning `HitCountGate.Admits`). Surfaced + fixed an engine flaw: per-stop `timeout` made a fast-hitting Suspend.None logpoint loop forever — both `WaitForPolicyStop` and `WaitForConditionalStop` are now deadline-based; probe 22 still passes (no regression). Future Suspend axis: per-thread; future location axes: data/field watchpoints, dependent, exception-through-policy.
