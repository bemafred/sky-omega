...
- [ ] breakpoints (`ICorDebugFunctionBreakpoint`), stepping (over/into/out), stack frames + variables (direct inspection, no func-eval v1) — produce stopping events that ride the model above; needs `ICorDebugModule`/`Function`/`Code`/`FunctionBreakpoint`/`Stepper`/`Frame`/`Value` + metadata resolution (the next increments)
