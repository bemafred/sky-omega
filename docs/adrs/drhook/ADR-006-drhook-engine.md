...
- [x] Roslyn integration: parse a C# expression → walk → predicate, for **primitive locals** (probe 22, finding 30) and **member access** (probe 25, finding 34 — `box.Size == 42`). The walker closes over `DebugSession`; a `MemberAccessExpressionSyntax` arm calls `TryEvalMemberCall` (getter func-eval'd on the runtime type). Remaining breadth: chained / `this`-from-arguments operands, the `ICorDebugType` path for strings/arrays/generics, and fields.
- [ ] **`BreakpointPolicy`** (finding 33 — breakpoint-types model): condition + hit-count gates, a log action reusing the Roslyn walker for `{expr}` interpolation, Suspend ∈ {All, None}. Conditions and logpoints fall out as two configs of one policy. Future axes (named, not YAGNI): per-thread suspend, data/field watchpoints, dependent breakpoints, exception-through-engine.
- [ ] **error feedback — tri-state condition outcome** (finding 35): `Func<IEvalContext,bool>` cannot express "couldn't evaluate," and the walker currently throws into the callback loop. Widen to `True/False/Fault(diagnostic)`; validate parse errors at *set* time via Roslyn `GetDiagnostics()` (span + message), reject before arming; fail loud once on a runtime `Fault` (`StopReason.ConditionError`), never silently resume. Interpolation is best-effort (a bad `{expr}` renders inline, the line still emits). A broken condition must never behave like a never-true condition.
- [ ] **structured log sink** (finding 35): widen `IDebugEventSink` from `OnEvent(string)` to a structured log event (timestamp, breakpoint id, rendered message, fault markers). Engine chooses **no** destination; host (`DrHook.Mcp`) tiers it: bounded ring buffer drained by a `drhook_log_drain` tool (default), file tee (opt-in, high volume), Mercury observations (structured, low-volume, valid-time-queryable). Volume guards: hit-count gate doubles as sampling; global byte cap; *visible* truncation accounting.
- [ ] **exception breakpoints — a distinct location axis** (finding 35): model = type filter (±subclasses) × first-chance/unhandled (`CorDebugExceptionCallbackType`) × optional condition on the exception object (reuses probe-24 member resolution). Two probe-gated unknowns before Engineering:
  - [ ] **probe 26** — confirm `ICorDebugManagedCallback2::Exception` fires with rich info (type + `CorDebugExceptionCallbackType`) on macOS/ARM64 CoreCLR. `ManagedCallback2` is a separately-QI'd vtable; the 38-method host wires `ManagedCallback` — extract `ManagedCallback2` slots/IID from cordebug.idl (no guessing) and confirm the callback is invoked.
  - [ ] **probe 27** — confirm func-eval works at an **Exception** stop (a different controller state than the Breakpoint stop probe 23 validated). Settles conditional exception breakpoints.
