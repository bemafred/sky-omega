...
- [ ] **clean-detach quiescence protocol** (`Stop` → `SetAllThreadsDebugState(SUSPEND)` → drain `HasQueuedCallbacks` → `Detach`) — probe 07 / finding 14 / [`docs/limits/drhook-clean-detach.md`](../../limits/drhook-clean-detach.md): `Detach` against a *busy* target races mscordbi's RC-event-thread flush and segfaults; the current `Dispose` is correct only for the quiet-detach case (probe 05). Confirmed root cause: killing the target first makes detach clean.
