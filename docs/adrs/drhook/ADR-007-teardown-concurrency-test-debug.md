...
- [ ] **Probe 47 — External target death during Borrowed observation (`drhook-external-target-death`).** Surfaced 2026-05-24 by Probe 44 phase C running under the new `--no-cache` protocol. Substrate's `Dispose` marches through Quiesce → TryResumeForDetach → Detach → Terminate even when the target is dead, racing mscordbi's RC thread's concurrent exit-work-item processing. Probe 44 phase C SIGSEGVs at cycle 2-4 against `44d76aa` AND HEAD; the original "10/10 PASS" in that commit was a stale-cache false-PASS. The dispatch-settle fix for Probe 42 does not reach this window. Substrate change: acquire `Process.GetProcessById(pid)` at Attach time for Borrowed (death-detection only, no kill ownership), have Dispose check `HasExited` before any mscordbi protocol op, skip Quiesce/Continue/Detach if dead. Production-critical scenario (NCrunch killing testhosts, user force-quits, OOM kills, target crashes). New probe + finding to follow. Phase 8 promotion remains unblocked — integration tests don't exercise external-death; the use case is for future production-observation work.
...
Completion: probes 41/42/43/45 pass at HEAD (Probe 42 closed 2026-05-24 by dispatch-settle in TryResumeForDetach per finding 65); **Probe 44 phase B passes; Probe 44 phase C re-opens as Probe 47** (external-target-death-during-Borrowed scenario — substrate currently crashes; needs target-death detection). `drhook-detach-exit-race` Mitigated (Attached-path TryResumeForDetach + dispatch-settle + Launched-path kill-first via finding 64's `AttachAndOwn`; kill-coincident-via-Borrowed remains Triggered until Probe 47 closes). `EngineAnomaly` surfacing validated end-to-end through MCP by Probe 41. **Phase 1 — Teardown + concurrency hardening + memory-model audit + stack-budget audit — substrate-correctness arc continues into Probe 47; informational-flood race closed.**
...
- [ ] Per-MCP-tool integration test. Each `[McpServerTool]` method gets a test driven end-to-end against an in-process target via `DebugSession.AttachAndOwn` (the probe targets in `poc/drhook-engine/` are the model — promote them via Phase 2's validated mechanism).
- [ ] Probes 57–61 promoted from `poc/` to `tests/DrHook.Engine.IntegrationTests/`.
- [ ] Probe 42 pre-existing regression (separately characterised in finding 64) investigated and resolved or scoped as a separate substrate change before promotion of probe 42 itself.
