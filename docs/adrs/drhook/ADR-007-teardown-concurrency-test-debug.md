...
- [ ] **Probe 42 — Dispose during the worker's `_resumeHandler(...)` call.** Force the race; characterise the failure mode; design the engine-side fix. *(Originally Done 2026-05-24 via finding 57 / 20/20 clean — **superseded 2026-05-24**: the original probe construction used `07-target.cs` whose throw/catch loop produced STOPPING callbacks that parked the worker at `_resume.Take()`, not inside `_resumeHandler`. Hypothesis and construction diverged. Replacement probe + dedicated `42-target.cs` (informational-only Thread.Start/Join flood, no exceptions) per [finding 65](../../../poc/drhook-engine/findings/65-probe42-redesign-regression.md). New probe: 50/50 PASS at 824e642 baseline; **SIGSEGV at HEAD** — real substrate regression introduced by `TryResumeForDetach` at commit 44d76aa (the same commit that landed Probe 44's substrate change). The 44d76aa commit's "re-ran Probe 42, still clean" was a stale-file-based-app-cache false-PASS per `feedback_filebased_app_stale_cache`. Phase 1 substrate-correctness arc **re-opens for Probe 42** until the regression is fixed and the replacement probe passes at HEAD. Phase 8 promotion remains unblocked — finding 64's `AttachAndOwn` integration tests use 1–2 sessions without informational flood, no exposure to the TryResumeForDetach × backlog-callback race.)*
...
Completion: probes 41/43/44/45 pass; **Probe 42 re-opened 2026-05-24** per finding 65 (substrate regression at HEAD revealed by hypothesis-aligned probe redesign — the original probe's PASS was for a different scenario than its name claimed). `drhook-detach-exit-race` Mitigated (Attached-path substrate change + Launched-path kill-first via finding 64's `AttachAndOwn`); `EngineAnomaly` surfacing validated end-to-end through MCP by Probe 41. **Phase 1 — Teardown + concurrency hardening + memory-model audit + stack-budget audit — substrate-correctness arc re-opens for Probe 42; remainder complete on macOS-arm64.**
