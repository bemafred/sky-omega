...
### bootstrap.ttl scoping

`docs/knowledge/bootstrap.ttl` is designed for the MCP store — it’s auto-loaded on first session with an empty MCP store to provide grounded context. Tutorials should be clear about this: the CLI store does not auto-load bootstrap.ttl, though users may load it manually for inspection via `LOAD <file:///path/to/bootstrap.ttl>`.
...
- Loading bootstrap.ttl into the MCP store: `mercury_update` with `LOAD <file:///path/to/docs/knowledge/bootstrap.ttl>`
...
### 1.4 `examples/Mercury.Examples/README.md`
...
- `bootstrap.ttl` is documented as MCP-store-scoped, not auto-loaded into CLI stores
