...
- [ ] **1.3** Enable and verify `RdfStarTests.QueryReifiedTriple_WithSparql`
  - **BLOCKED**: Test runs TWO queries in single async method
  - Combined stack usage (~50KB) exceeds limits even with NoInlining
  - Requires Phase 2 (QueryBuffer migration) to fully resolve
...
- [ ] **2.1** Audit all `Query` struct usage in executor paths
  - Identify all locations where `Query` is copied
  - Document stack frame sizes

- [ ] **2.2** Extend `QueryBuffer` to hold all pattern types
  - Currently holds basic patterns
  - Add support for: ExistsFilters, GraphClauses, SubSelects, ServiceClauses

- [ ] **2.3** Create `QueryBuffer.FromQuery(Query q)` factory
  - Copies inline storage to pooled heap
  - Returns lightweight handle

- [ ] **2.4** Update `QueryExecutor` to use `QueryBuffer`
  - Accept `QueryBuffer` in constructor (overload)
  - Access patterns via buffer, not inline struct

- [ ] **2.5** Update `SparqlParser` to emit to `QueryBuffer` directly
  - Avoid intermediate `Query` struct
  - Zero-copy from parse to execution
...
| Stack headroom | Complex queries use < 500KB stack | Partial (single queries OK, multiple queries in async method still problematic) |
...
- Phase 2 requires significant refactoring of parser/executor interface
