...
**Remaining for Accepted** (breadth + scale, not mechanism): run the now profile-parameterized, temporal-aware differential over a larger corpus — **WDBench** is the right intermediate-scale dogfood (loadable in Cognitive for broad + temporal) — and validate the planner-spike finding **at scale on Reference** (the 21.3 B Wikidata on disk, or WDBench-scale). The materialization risk is resolved for the regression (aggregate) case; the only remaining materialization work is the *non-regression* owned-buffer spill for ORDER BY / DISTINCT at Reference scale, which both paths share and which does not gate the cutover. Strategy: **Cognitive for broad correctness, Reference for scale performance.**
