...
- [ ] **Step 4 — uniform executor walk** (active-graph parameter; folds in the LIMIT-pushdown requirement from `ck:obs-graph-limit-pushdown`) → cutover; delete `FromMaterializedWithGraphContext` and the `QueryExecutor.Graph` divergence; flip the mirror-gate `false` baselines to `true`.
