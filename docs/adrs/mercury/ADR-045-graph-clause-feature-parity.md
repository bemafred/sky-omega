...
- Candidate instance under investigation, found by dogfood 2026-06-07 (`ck:obs-pname-filter-graph-divergence`): on the top-level-GRAPH path, a **FILTER equality on the *subject* variable against a prefixed name returns nothing** — `GRAPH <g> { ?s ?p ?o . FILTER(?s = ck:foo) }` matches 0, while the identical `FILTER(?s = <full-iri>)` matches, *and* the same prefixed name on the predicate or object position (`FILTER(?p = ck:status)`, `FILTER(?o = ck:Type)`) matches. So it is a **subject-position-specific** PNAME-comparison defect, not the general "PNAMEs don't resolve in GRAPH filters" the initial recording over-claimed. Proven behaviourally; mechanism and GRAPH-vs-default attribution are **open** (default graph empty here, so the control is untestable) — not yet confirmed as a divergence-class instance. The unified path makes it moot regardless.
