...
**Status:** Completed — 2026-05-13. **Production-validated at scale by cycle 10 Phase 3 r4 on 1.7.57** with first-ever instrumented MPHF construction measurement on 4,005,235,528 atoms: **54 m 29 s** total wall-clock (54 m 12 s build + 17 s file write), **25 iterative levels** (converged at level 24), **0 dense fallback keys** (the 1.7.55 dense surface is defense-in-depth that did not need to fire at this N), **placement_ratio = 0.6065** held across levels 0-7 — exact match to BBHash theoretical `1 − e^(−1/γ)` for γ=2.0. Output files: 1.75 GB `atoms.mphf` blob + 16.0 GB `atoms.idx` translation table (uint32 per atom, mmap'd). Construction time is within 1 % of the cycle 10 plan's "+~55 min MPHF" budget. MPHF event surface (1.7.56) + listener wire-through fix at `QuadStore.RebuildMphf` (1.7.57) deliver per-level / dense-fallback / start / completed events on both the bulk-load finalize path and the standalone `--rebuild-mphf` recovery path. Substrate output identity preserved vs cycle 9 (same atom set, deterministic). Validation: [docs/validations/cycle10-phase3-r4-21b-2026-05-12.md](../../validations/cycle10-phase3-r4-21b-2026-05-12.md).

**Original acceptance:** 2026-05-09. Reviewed pre-implementation with four implementation-detail clarifications (verification cost, concurrency contract, construction memory budget, file-format magic bytes). Updated 2026-05-09 with a fifth clarification surfaced during B3 implementation start: BBHash maps each key to a unique position in `[0, N)` determined by its hash algorithm — *not* our sorted-position atom ID. A **translation table** `T[mphf_pos] = sorted_pos` is required to convert MPHF output to our atom ID. At 4 B atoms × uint32 entries = ~16 GB additional storage (mmap'd, paged-in on demand). See "Translation table" section below.
