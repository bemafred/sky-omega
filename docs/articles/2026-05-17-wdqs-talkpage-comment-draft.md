...
For context: we have published measurements of the full Wikidata RDF dump (`latest-all.ttl.bz2`, 21.3 billion triples) loaded onto a single consumer laptop (Apple MacBook Pro, M5 Max, 128 GB unified memory, 8 TB SSD) using a BCL-only .NET substrate called Mercury, with 100% W3C SPARQL 1.1 core conformance and queryable end-to-end in 23 hours 57 minutes — a number that includes the trigram text index Mercury builds unconditionally for SPARQL `text:match`; the apples-to-apples comparable against QLever-class systems that omit text indexes by default is 15 hours 26 minutes. We also published a paired truthy measurement (8.17 billion triples, 14 hours 13 minutes with trigram / 6 hours 49 minutes without) so that comparisons against published truthy-based benchmarks (QLever, WDBench) are explicit about which dataset each system loaded and which feature surface each substrate carries.
