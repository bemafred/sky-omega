...
For context: we have published measurements of the full Wikidata RDF dump (<code>latest-all.ttl.bz2</code>, 21.3 billion triples) loaded onto a single consumer laptop (Apple MacBook Pro, M5 Max, 128 GB unified memory, 8 TB SSD) using a BCL-only .NET substrate called Mercury, with 100% W3C SPARQL 1.1 core conformance and queryable end-to-end in 23 hours 57 minutes — a number that includes the trigram text index Mercury builds unconditionally for SPARQL <code>text:match</code>; the apples-to-apples comparable against QLever-class systems that omit text indexes by default is 15 hours 26 minutes. We also published a paired truthy measurement (8.17 billion triples, 14 hours 13 minutes with trigram / 6 hours 49 minutes without) so that comparisons against published truthy-based benchmarks (QLever, WDBench) are explicit about which dataset each system loaded and which feature surface each substrate carries.

The validation arc, the per-cycle JSONL metric artifacts, and an explicit comparison-plane memo documenting which numbers in the broader RDF literature compare on the same workload versus different workloads are all in the public repository at [https://github.com/bemafred/sky-omega github.com/bemafred/sky-omega] — specifically <code>[https://github.com/bemafred/sky-omega/tree/main/docs/validations docs/validations/]</code>, <code>[https://github.com/bemafred/sky-omega/tree/main/docs/articles docs/articles/]</code>, and the comparison-plane memo at <code>[https://github.com/bemafred/sky-omega/blob/main/docs/memos/2026-04-30-latent-assumptions-from-qlever-comparison.md docs/memos/2026-04-30-latent-assumptions-from-qlever-comparison.md]</code>.
