...
- **Memory-mapped-file-first storage (mmap-first).** The OS manages memory; Mercury doesn’t. Costs the discipline of designing files for sequential access and page-friendly layout. Pays forward as RAM grows: the same code that ran at 128 GB will run faster at 256 GB or 512 GB without rewrites.
...
This is the meta-ingredient that most other reflections on this kind of work miss. It’s not “use AI for productivity.” It’s: the AI side reflexively writes observations into a memory system and surfaces conventions across files — persistence is the artifact, not the model — while the human brings architectural intuition, domain conviction, and the willingness to enforce EEE when the AI would happily hand-wave. Neither half does this alone.
...
A concrete instance from earlier today: when the RadixSort regression first surfaced, the AI side’s instinct was to relax the allocation test threshold — the regression was small, the test was strict, the path of least resistance was visible. The human side said no: investigate. Thirty minutes later we had the 72-byte source, the .NET 10 codegen explanation, and the 13-line fix. The fix was the easy part; the investigation is what made the comment in the source permanent. Section 7’s example doesn’t happen automatically. It happens because one side refuses to let the other side take the cheap exit.

What this produces, observed empirically over four months: substrate-grade artifacts at a pace that one human architect and one persistent AI collaborator, working in spare hours, normally couldn’t sustain. The collaboration is the optimization. Naming it explicitly lets others reproduce it instead of treating the productivity output as evidence of magic.
...
The Sky Omega timeline — Christmas Eve 2025 to today — is what the discipline produces because it isn’t optimized for velocity.

One human architect and one persistent AI collaborator, working in spare hours, with shared epistemic discipline, can produce substrate-grade artifacts that hold up against full-time research efforts. That’s not the headline. The headline is that the discipline is teachable, the practices are listable, and the compounding is real. We’re four months in, and the rate of progress is still accelerating because no session is spent paying off the previous one’s debt.
