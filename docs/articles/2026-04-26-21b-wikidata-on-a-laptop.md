...
**“You’d need months of engineering.”** Days to weeks of focused work. A human-AI engineering pair, sustained context across the arc. Solution parsimony made it tractable.
...
|3    |Hardware-accelerated XxHash3 (first-party Microsoft `System.IO.Hashing`, NEON on Apple Silicon)   |5-15% on hash hot path |Latent — small change        |
...
Compounding conservatively, Rounds 1-4 would put a fully-tuned 21.3 B run somewhere around **15-25 hours on the same laptop**.

QLever (Hannah Bast et al., Univ. Freiburg, mature C++ codebase) reports substantially faster full-Wikidata loads on comparable consumer hardware. Decomposing where their time goes vs Mercury’s — vocab sort, permutation builds, text index, durability — is the work of Round 2. Memory is memory, NVMe is NVMe; optimization is about avoiding unnecessary work, and the gap between 85h and their published numbers is a gap in *what work each system is doing per triple*, not in physics. That decomposition is what Round 2 starts with.

**Mercury at Round 1 is competitive enough to demonstrate that the architectural bets work, with seven documented rounds ahead, each anchored in measurement.** The honest framing isn’t faster-than-X — it’s: here is the gap, here is the plan to close it, here are the measurements that will adjudicate.
...
The "we" throughout this article is also load-bearing. The work was a sustained human-AI engineering collaboration — architectural decisions, debugging, validation arcs, the limits register itself, all developed in dialogue with shared epistemic discipline. Sky Omega is built for that kind of partnership; the engineering that built Mercury is itself a demonstration of what Sky Omega is for.
