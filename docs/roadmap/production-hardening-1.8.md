...
- ✅ ADR-040 implementation — readahead memory adaptive sizing. Parts 1+4 shipped 1.7.63 (adaptive `bufferSize` at MergeAndWrite start via `ProcessMemoryProbe` + 0.25 budget fraction, halving down to 256 KiB; `ReadAheadBudgetEvent` JSONL emission). Parts 2+3 shipped 1.7.64 (lazy back-buffer allocation; eager per-chunk teardown on exhaustion via `ChunkReader.MoveNextReadAhead`). 550 Storage tests green; end-to-end verified on 10M-triple bulk-load. Periodic `ReadAheadFootprintSampleEvent` deferred as a follow-up enhancement.
- ✅ ADR-042 implementation — MPHF construction memory adaptive sizing. All five parts shipped: Parts 1+4 in 1.7.60 (range iterator at level 0 + Span-based `GetKey` API), Parts 2+3 in 1.7.62 (mmap-backed streaming translation + re-hash second pass eliminating `keyPositions`), Part 5 in 1.7.64 (host-adaptive `MphfMemoryBudgetEvent` at `BuildMphfFiles` start). Projected peak MPHF-phase RSS at 4 B atoms: ~100 GB → ~15 GB; GC churn: ~770 GB → 0; persistent translation: 32 GB anonymous-mem → 16 GB mmap'd.
...
- ✅ N-Triples parser per-triple profile + decision — shipped 2026-05-16 in 1.7.59. Peek-inlining asymmetry vs Turtle closed; +6.0% end-to-end / +7.4% steady-state at 10M-triple scale. ~25% steady-state gap remains, characterized as grammar-inherent. Options B (vectorized IRI scan) + C (`ConsumeNonNewline`) deferred Latent. See [ntriples-parser-profile-2026-05-16.md](../validations/ntriples-parser-profile-2026-05-16.md).
...
- [x] ADR-040 implementation — readahead memory adaptive sizing. *(Parts 1+4 shipped 1.7.63; Parts 2+3 shipped 1.7.64; 550 Storage tests green; end-to-end verified.)*
...
- [x] ADR-042 implementation — MPHF construction memory adaptive sizing. *(All five parts shipped 1.7.60 → 1.7.64; projected peak RSS at 4 B atoms: ~100 GB → ~15 GB; output byte-equivalence preserved.)*
...
- [x] N-Triples parser per-triple profile + decision — *(Shipped 1.7.59, 2026-05-16. Peek-inlining: +6.0% end-to-end / +7.4% steady-state. 25% remainder grammar-inherent + deferred Latent.)*
