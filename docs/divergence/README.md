...
### S1 · RDF format parsers reimplement atom-minting canonicalization 3–6× — and already disagree  ✓ `live`
The same source triple ingested as different formats can store a **different atom**. Sub-concerns:
- **S1a escape-decode** — `\n \t \" \\ \uXXXX \UXXXXXXXX`. Copies: `NTriples/NTriplesStreamParser.cs:551,580`; `Turtle/TurtleStreamParser.Buffer.cs:395,423,456,519,555`; `TriG/TriGStreamParser.cs:1513,1548`; `NQuads/NQuadsStreamParser.cs:658,694`; plus the correct shared one `Sparql/Execution/LiteralForm.cs:181`. **They disagree:** N-Triples validates `\U` via `Rune.TryCreate` (rejects surrogates + `>U+10FFFF`); Turtle has an explicit `>0x10FFFF` guard; TriG/N-Quads ◦ lack the upper guard → different exception (raw `ArgumentOutOfRangeException`) for the same malformed input.
