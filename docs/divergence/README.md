...
- ~~**S1b IRI resolution**~~ (the biggest atom-identity risk) — **RDF-family CONVERGED 2026-06-20.** Turtle (both copies), TriG, and RDF-XML now resolve relative IRIs through the single `Rdf/RdfIri.cs` — a literal RFC 3986 §5.2 implementation grounded in the spec's §5.4 normative vectors (`RdfIriResolutionTests`, 40 vectors). Replaced BCL `new Uri` (over-normalised: RDF-XML's `.AbsoluteUri` gave `http://g/` where Turtle's fix-up gave the RFC-correct `http://g` — *different atoms for the same IRI*) and deleted TriG's hand-rolled cluster (~340 lines). N-Triples/N-Quads require absolute IRIs (no relative resolution — out of scope). **JSON-LD CONVERGED 2026-06-20** — the EEE assessment was *resolved empirically, not assumed*: JSON-LD's `ResolveRelativeIri` is pure RFC 3986 base resolution (the JSON-LD-specific expansion — compact IRIs, `@vocab`, terms — lives in the *caller*, untouched), so it was a 4th hand-rolled RFC copy, not a distinct concern. Routed onto `RdfIri.Resolve` (its dead `ParseIriComponents`/`RemoveDotSegments` deleted); JSON-LD suite stayed 525/0/6. *Original evidence:* Turtle `Buffer.cs:636`/`Terminals.cs:811`, RDF-XML `:914`, TriG `:2112`, JSON-LD `Iris.cs:317`.
- ~~**S1c numeric/boolean → xsd typing**~~ — **CONVERGED 2026-06-20.** The xsd datatype IRI constants, the boolean literal forms, and the selection rule (exp⇒double, dot⇒decimal, else⇒integer) now live in `Rdf/RdfNumeric.cs`; Turtle, TriG, and `LiteralForm.CanonicalizeNumericOrBoolean` all use it. The per-grammar *classification* (computing has-exponent/has-decimal while consuming source) stays in each parser — legitimately distinct. Locked by `RdfNumericConvergenceTests` (a numeric/boolean types identically across Turtle, TriG, and the SPARQL canonicalizer, so a SPARQL constant matches an ingested atom). *Original:* inlined ~4× in Turtle `Terminals.cs`, once in TriG, and in `LiteralForm`.
- ~~**S1d** code-point→UTF-16 appender~~ — **CONVERGED 2026-06-20.** The 3 hand-rolled surrogate-pair copies (TriG/N-Quads/RDF-XML) plus Turtle's two inline `Rune` versions now route through `RdfEscape.EncodeUtf16`; N-Triples' `AppendRune` (a clean buffer-optimised `Rune.EncodeToUtf16`, not surrogate math) is left. ~~**S1e** "is IRI absolute?" predicate~~ — **CONVERGED 2026-06-20** (resolution gates). Turtle (both), TriG, RDF-XML, and JSON-LD (`IsAbsoluteIri` → one-line delegation, keeping its 9 internal callers) now decide absolute-vs-relative via the one `RdfIri.HasScheme` (RFC 3986 scheme syntax), replacing `Uri.IsWellFormedUriString` / `Contains("://")` / `Contains("://")||"urn:"` / a per-parser colon scan — which disagreed (`mailto:x`, `urn:x` have a scheme but no "//"), an atom-identity hazard since it gates *which* refs get resolved. N-Triples/N-Quads' `hasScheme` is left as a **legitimately distinct concern** (a lenient absolute-*presence* validation fused into the IRIREF parse loop — N-Triples [8] IRIREF does not enforce scheme syntax — not a resolution gate; forcing the stricter test would reject IRIs it intentionally accepts).
...
- **FilterEvaluator** — full logical grammar `Or→And→Unary(!)→Primary→Comparison→Term`, `IN`, and **52** functions incl. the 11 boolean predicates (`CONTAINS`/`REGEX`/`STRSTARTS`/`STRENDS`/`LANGMATCHES`/`sameTerm`/`text:match`/`isIRI`…). Returns `bool` (EBV) at the top; only the primary/function layer returns `Value`. **NO arithmetic** (`+`/`-`/`*`/`/`).
- **BindExpressionEvaluator** — `Comparison→Additive→Multiplicative→Unary→Primary` with **arithmetic**, and **41** functions. Returns `Value`. **NO `||`/`&&`/`!`**, and lacks the 11 boolean predicates → `BIND(CONTAINS(?a,?b) AS ?x)` silently unbinds.

So BIND has TWO gaps (no `||`/`&&`, no boolean predicates) and FILTER has one (no arithmetic). The ~40 shared functions are **parser-coupled** (each parses its own args inline from the source span), so they cannot be lifted into a pure shared library — the only real convergence is a **full grammar union into one `Value`-producing evaluator**, with FILTER deriving `bool` via EBV (`CoerceToBool`). This is the largest and most conformance-sensitive convergence (hundreds of W3C FILTER+BIND expression tests).

**The spec settles it — they MUST be unified (SPARQL 1.1, grammar verbatim from the W3C Rec):** both FILTER and BIND use the *one* `[110] Expression` production. `[60] Bind ::= 'BIND' '(' Expression 'AS' Var ')'`; FILTER's `[68] Constraint` is a `BrackettedExpression ::= '(' Expression ')'`. `[110] Expression` expands to the full chain — `[111] ConditionalOrExpression` (`||`) → `[112]` (`&&`) → `[113] ValueLogical` → `[114] RelationalExpression` (`= != < > <= >=`, `IN`, `NOT IN`) → `[115] NumericExpression` → `[116] AdditiveExpression` (`+ -`) → `[117] MultiplicativeExpression` (`* /`) → `[118] UnaryExpression` (`! + -`) → `[119] PrimaryExpression` (`BrackettedExpression | BuiltInCall | iriOrFunction | RDFLiteral | NumericLiteral | BooleanLiteral | Var`). There is **no** FILTER-only or BIND-only expression — it is one grammar. The *only* difference is post-evaluation consumption: an expression evaluates to an RDF term **or an error** (§17), then **FILTER** applies EBV (§17.2.2; error → eliminate the solution) and **BIND** binds the value ("If the evaluation of the expression produces an error, the variable remains unbound for that solution but the query evaluation continues" — §18 quote, fetched). So the current split is **non-conformant**: each evaluator implements a *different half* of `[110]` (FilterEvaluator omits `[116]/[117]` arithmetic; BindExpressionEvaluator omits `[111]/[112]` `||`/`&&` and `[118]` `!`). Converging is a **conformance fix**, not just cleanup.

**Verdict: TRUE divergence; the spec mandates one evaluator. ADR-worthy — design before implementing.** Path: implement `[110] Expression` once as a `Value`-producing recursive descent (RDF-term-or-error); `FILTER Evaluate()` = `CoerceToBool` (EBV, error→false); BIND = bind value, error→unbound. Build it by giving FilterEvaluator the arithmetic layer it lacks (→ full `[110]`), adding `EvaluateToValue()`, routing the 9 BIND call sites, deleting `BindExpressionEvaluator` — each step W3C-verified.
...
1. ~~**S1 — shared RDF term canonicalizer**~~ — ✅ **COMPLETE 2026-06-20.** All sub-items converged: S1a escape (`RdfEscape`), S1b IRI resolution incl. JSON-LD (`RdfIri`), S1c numeric/boolean (`RdfNumeric`), S1d code-point encoding, S1e absolute-IRI test (`RdfIri.HasScheme`). New shared primitives under `src/Mercury/Rdf/`. N-Triples/N-Quads' lenient `hasScheme` validation left as legitimately distinct. The atom-minting routines are now single-sourced across N-Triples/N-Quads/Turtle/TriG/RDF-XML/JSON-LD and the SPARQL `LiteralForm`.
