...
- ~~**S1b IRI resolution**~~ (the biggest atom-identity risk) — **RDF-family CONVERGED 2026-06-20.** Turtle (both copies), TriG, and RDF-XML now resolve relative IRIs through the single `Rdf/RdfIri.cs` — a literal RFC 3986 §5.2 implementation grounded in the spec's §5.4 normative vectors (`RdfIriResolutionTests`, 40 vectors). Replaced BCL `new Uri` (over-normalised: RDF-XML's `.AbsoluteUri` gave `http://g/` where Turtle's fix-up gave the RFC-correct `http://g` — *different atoms for the same IRI*) and deleted TriG's hand-rolled cluster (~340 lines). N-Triples/N-Quads require absolute IRIs (no relative resolution — out of scope). **JSON-LD (`JsonLd/JsonLdStreamParser.Iris.cs:154,317`) NOT routed — pending EEE assessment:** JSON-LD 1.1 IRI *expansion* (context `@base`/`@vocab`/term definitions) is its own spec algorithm, not necessarily identical to RFC 3986 §5.2, so it may be a legitimately distinct concern rather than a divergence — do not route onto `RdfIri` without confirming the semantics match (6 W3C tests already skipped there). *Original evidence:* Turtle `Buffer.cs:636`/`Terminals.cs:811`, RDF-XML `:914`, TriG `:2112`, JSON-LD `Iris.cs:317`.
