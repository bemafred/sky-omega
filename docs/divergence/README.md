...
So BIND has TWO gaps (no `||`/`&&`, no boolean predicates) and FILTER has one (no arithmetic). The ~40 shared functions are **parser-coupled** (each parses its own args inline from the source span), so they cannot be lifted into a pure shared library — the only real convergence is a **full grammar union into one `Value`-producing evaluator**, with FILTER deriving `bool` via EBV (`CoerceToBool`). This is the largest and most conformance-sensitive convergence (hundreds of W3C FILTER+BIND expression tests). **Verdict: TRUE divergence; ADR-worthy — design before implementing.** Recommended path: make FilterEvaluator the superset (add the arithmetic layer it lacks), add `EvaluateToValue()` (Value-returning grammar; FILTER `Evaluate()` becomes `CoerceToBool(EvaluateToValue())`), route the 9 BIND call sites, delete `BindExpressionEvaluator` — each step W3C-verified.
