...
**Current release: [Sky Omega 1.8.9](#189---2026-06-26)** — released 2026-06-26; **DrHook debug-state visualization (ADR-012 Phases 1–2).** A surface-agnostic debug-state model the engine publishes over a Unix-domain **rendezvous socket** to human-launched views — the layered `DrHook.Wire` (protocol) / `DrHook.Viz` (client) / `DrHook.Viz.Console` (`drhook-viz-console`) stack + the transport, wired into the MCP host and **dogfooded end-to-end** (a separate view process rendered a live agent session; the view is itself the first proper DrHook debuggee). Resolves ADR-012 Q1/Q4/Q6/Q7 + the lifecycle half of Q3. See [docs/releases/1.8.9.md](docs/releases/1.8.9.md). Previous: [Sky Omega 1.8.8](docs/releases/1.8.8.md) — released 2026-06-25; **DrHook runtime-version-lock guard** — a debugger process can host the `mscordbi`/DAC of only one runtime major, so `DebugSession.GuardRuntimeLock` now refuses a mismatched-runtime launch/attach up front with an actionable error instead of an opaque `0x80131C3C` (finding 86). Previous: [Sky Omega 1.8.5](#185---2026-06-22) — released 2026-06-22; **DrHook navigable inspection + the Mercury silent-drop fix it surfaced.** A dogfooding release: DrHook's eager variable inspection (which crashed the engine on large object graphs via a native unwinder fault) was rebuilt lazy + navigable (`drhook_locals` one level + `hasChildren` + the new `drhook_expand`; [ADR-007](docs/adrs/drhook/ADR-007-teardown-concurrency-test-debug.md) inspection item + [ADR-013](docs/adrs/drhook/ADR-013-value-type-refstruct-inspection.md) D1/D2), then pointed at Mercury — where it traced a silent data-loss bug to one line: `BindingTable.Bind` dropped string literals ≥ 1023 chars from SELECT result rows on buffer overflow ([ADR-050](docs/adrs/mercury/ADR-050-growable-result-binding-buffer.md), Completed — the buffer now grows, bounded by `MaxAtomSize`). DrHook then validated the fix live. Full suite **4,699 / 0 failed / 6 skipped**; 119/119 DrHook engine tests. The hunt also surfaced DrHook's next gap ([ADR-014](docs/adrs/drhook/ADR-014-inspection-fault-containment.md), Proposed). See [docs/releases/1.8.5.md](docs/releases/1.8.5.md). Previous: [Sky Omega 1.8.4](#184---2026-06-21) — released 2026-06-21; **Mercury convergence: one execution path, one expression evaluator.** [ADR-049](docs/adrs/mercury/ADR-049-unified-expression-evaluator.md) unified FILTER and BIND onto the single SPARQL `[110] Expression` evaluator `FilterEvaluator.EvaluateToValue` (FILTER via `CoerceToBool`); `BindExpressionEvaluator` **deleted (2,336 lines)**; divergence S2 closed. Also first-released here (landed on `main` after 1.8.3): [ADR-047](docs/adrs/mercury/ADR-047-default-path-cutover.md) — the unified `TreeJoinExecutor` is the sole SPARQL execution path, the ~14 K-line slot-operator engine deleted — and the **S1 cross-format canonicalizer convergence** onto shared `RdfEscape` / `RdfIri` / `RdfNumeric`. The merge was strictly incremental, zero reverts, and corrected real conformance divergences in *both* former expression evaluators (date/time timezone-conversion, `STRDT`/`STRLANG`/`DATATYPE`, `@lang`/datatype literal parsing) — the spec, not either implementation, was the oracle. Full suite **4,699 / 0 failed / 6 skipped**; W3C conformance unchanged (421/421 SPARQL Query). See [docs/releases/1.8.4.md](docs/releases/1.8.4.md). Previous: [Sky Omega 1.8.3](#183---2026-06-06) — released 2026-06-06; **DrHook lifecycle, console isolation, and the 22-tool surface.** ADR-010 Tier 1 tool-surface rename to IDE-debugger convention + ADR-011 `stop` / `detach` / `kill` lifecycle triad with a launched debuggee's console output isolated from the MCP channel; the lifecycle is now complete for both Owned (launched) and Borrowed (attached) targets (findings F-010-1, F-010-2). One Mercury correctness fix rides along: aggregation dropped on the top-level GRAPH execution path (+4 regression tests; 421/421 conformance held). See [docs/releases/1.8.3.md](docs/releases/1.8.3.md). Previous: [Sky Omega 1.8.2](#182---2026-05-23) — released 2026-05-23; **DrHook substrate-independence reached.** `src/DrHook/` retired entirely; `DrHook.Engine` (BCL + P/Invoke + source-gen COM) is now Sky Omega's sole .NET runtime-observation substrate. The Phase 3 substrate arc named in ADR-006 closes here — every load-bearing capability validated by probes 02–40 (39 PASS + 1 documented PARTIAL on macOS/arm64). `libdbgshim` bundled per-RID via NuGet; no consumer relies on `DBGSHIM_PATH`. The successor [ADR-007](docs/adrs/drhook/ADR-007-teardown-concurrency-test-debug.md) (Proposed 2026-05-23) sequences the production-suitability work (teardown + concurrency hardening; substrate-aligned test-runner debugging; integration-test mechanism characterization; cross-platform validation campaign). See [docs/releases/1.8.2.md](docs/releases/1.8.2.md). Previous: [Sky Omega 1.8.1](#181---2026-05-22) — `DrHook.Mcp` stepping path switched from netcoredbg-DAP to `DrHook.Engine`; substrate work that enabled the switchover (BreakpointPolicy unification, exception breakpoints with subclass filtering, func-eval at exception stops, Roslyn-walker-driven conditional/interpolated expressions, `DebugSession.Pause`/`Launch`, persistent exception filter) shipped across probes 23–32. See [docs/releases/1.8.1.md](docs/releases/1.8.1.md). Previous: [Sky Omega 1.8.0](#180---2026-05-17) — released 2026-05-17; **cognitive-layers entry point.** The 1.7.x substrate-hardening line closes here. 1.8.0 is the boundary between substrate work and cognitive work — no new substrate features in this release, only the *closure* of the substrate-hardening arc and the *framing* for what cognitive-layer work begins next (Lucy → James → Mira → Sky over 1.8.x). All 18 Mercury ADRs through ADR-044 are Completed or explicitly Deferred. Substrate posture at 1.8.0 cutover: **W3C SPARQL 1.1 100/100 + Turtle/N-Triples/N-Quads/TriG/RDF-XML 100% + JSON-LD 100% of in-scope**, **4,522 / 0 failed / 6 skipped** Mercury tests, **0 substrate failures across 8,564 unique query × substrate executions** on the 1.7.x line, three paired production runs (21.3 B full + 8.17 B truthy + 150 M WGPB) on a single M5 Max laptop. See [docs/releases/1.8.0.md](docs/releases/1.8.0.md) for the substrate completion arc and [docs/roadmap/cognitive-layers.md](docs/roadmap/cognitive-layers.md) for what 1.8.x begins. Previous: [Mercury 1.7.74](#1774---2026-05-17) — released 2026-05-17; **[ADR-043](docs/adrs/mercury/ADR-043-metric-emission-decoupling.md) Metric Emission Decoupling shipped. Three-part fix bounds JSONL artefact staleness on shared-disk configurations: periodic `FileStream.Flush()` (5s default, configurable), `MetricEmissionThrottle` helper applied at merge emit site, `ProgressEmissionMinInterval` default 30s → 5s for rebuild/drain emit sites, and a new operator runbook for the separate-disk `--metrics-out` pattern. Cycle 9 trigram drain's 2-hour staleness symptom is structurally bounded. Also: `observability-discipline-systematic-not-reactive` limit Resolved at the 1.8.0 boundary (structural cure lives in James; Martin mitigates manually until cognitive layer is active).** Previous: [Mercury 1.7.73](#1773---2026-05-17) — released 2026-05-17; **[ADR-044](docs/adrs/mercury/ADR-044-sparql-update-literal-canonicalization.md) SPARQL literal canonicalization shipped: SPARQL `INSERT/DELETE DATA`, `INSERT/DELETE WHERE`, FILTER, BIND, and pattern literals now canonicalize to the same wrapped-decoded form Turtle/N-Triples/N-Quads/TriG already produce. Cross-format ingestion of the same logical RDF triple now produces byte-identical atoms. Pre-existing `FilterEvaluator.CompareEqual` wrap-vs-unwrap equality bug brought into ADR scope and fixed; `FILTER(?o = "literal")` now normalizes lexical form + lang tag + datatype per SPARQL spec. 14 cross-form tests in `CrossFormCanonicalizationTests` + 38 unit tests in `LiteralFormTests` validate H1-H4. Full suite 4,515 / 0 failed / 6 skipped.** Previous: [Mercury 1.7.72](#1772---2026-05-17) — **fix: `GetLexicalForm` skips escaped quotes; CONTAINS/STRSTARTS/UCASE/LCASE no longer truncate literals at `\"`**. `BTreeFile` extracted to `Mercury.Runtime` alongside `PageCache` (relocated, namespace `SkyOmega.Mercury.Runtime`, visibility `public`). The four concrete index classes (`TemporalQuadIndex`, `VersionedQuadIndex`, `ReferenceQuadIndex`, `MinimalQuadIndex`) now own only the B+Tree algorithm + key/entry layouts; FileStream + mmap + page allocation + metadata header are delegated. ~600 lines of boilerplate eliminated. 592 Storage tests + 4454 total Mercury tests green. Production substrate continues to be validated by 1.7.57's **three paired measurements on the same substrate generation**:
...
**DrHook debug-state visualization — a surface-agnostic model published over a Unix-domain socket to human-launched views** ([ADR-012](docs/adrs/drhook/ADR-012-debug-state-surfaces.md) Phases 1–2). See [docs/releases/1.8.9.md](docs/releases/1.8.9.md).

### Added

- **The debug-state model + transport** (ADR-012 Phase 1–2). The engine assembles a self-contained `DebugStateSnapshot` (+ a `DebugStateDelta` stream atop `IDebugEventSink`), and `DrHook.Engine/Transport/DebugStateServer` publishes snapshot-on-connect + the delta stream over a fixed Unix-domain **rendezvous socket** (`~/Library/SkyOmega/drhook/session.sock`). `EngineSteppingSession` wires it as a 4th `CompositeEventSink` member and publishes a snapshot after each stop — the request-thread driver owns capture, so the transport never calls into `DebugSession` (no transport↔stepping race); best-effort (a transport fault never breaks a debug response). `DebugStateServer` is restartable for the per-session start/stop lifecycle.
- **A layered visualization client**, mirroring `Mercury.Abstractions ← Mercury.Sparql.Tool ← Mercury.Cli.Sparql`: **`DrHook.Wire`** (zero-dependency NDJSON protocol contract — `Wire*` DTOs + source-gen `WireCodec` + `WireRendezvous`), **`DrHook.Viz`** (BCL-only client library — `DebugStateClient` + `DebugStateClientModel` + `IDebugStateView`, references only `DrHook.Wire`), **`DrHook.Viz.Console`** (`drhook-viz-console`, the first console view and the first proper DrHook debuggee).
- **Tests:** 4 `DrHook.Wire.Tests` (codec round-trip) + 6 `DrHook.Viz.Tests` (the client end-to-end against a live server + console rendering); `DrHook.Engine.Tests` **135 → 154** (the snapshot model + the transport server, incl. a restart test). Dogfooded end-to-end through the real DrHook MCP.

### Resolved (ADR-012 open questions)

- **Q1 topology** — MCP-server-as-host (no broker); views are connect-in clients.
- **Q4 wire format** — newline-delimited JSON via `System.Text.Json`, through a thin wire-DTO layer.
...
- **Q3 (lifecycle half)** — terminating a view never affects the ongoing session.

Phases 3–6 (the `(hypothesis, observation)` braid, the full TUI, command-in control, the Avalonia GUI) and Q2/Q5 + the command half of Q3 remain open.

## [1.8.5] - 2026-06-22

**DrHook navigable inspection + the Mercury silent-drop fix it surfaced** (a dogfooding release). See [docs/releases/1.8.5.md](docs/releases/1.8.5.md).

### Fixed

- **Mercury: silent result-binding drop** ([ADR-050](docs/adrs/mercury/ADR-050-growable-result-binding-buffer.md)). `BindingTable.Bind` silently returned on string-buffer overflow, so any string literal whose lexical form exceeded the fixed 1 KB result buffer (i.e. ≥ 1023 chars) vanished from SELECT result rows (the row came back without the binding) — a read-back bug that also corrupted long `ck`/memory writes. The buffer now GROWS on demand via `ArrayPool<char>.Shared` (steady state stays zero-GC; growth is pooled), bounded by the store's `MaxAtomSize` (the literal-size policy, now specified — RDF/SPARQL are unbounded and no surveyed provider publishes a small cap). Verified: `tools/repro-literal.cs`, `LongLiteralRoundTripTests` (500–100,000 chars), full suite 4,699/0/6, and live under DrHook.

### Changed

- **DrHook: lazy, navigable variable inspection** ([ADR-007](docs/adrs/drhook/ADR-007-teardown-concurrency-test-debug.md) inspection item + [ADR-013](docs/adrs/drhook/ADR-013-value-type-refstruct-inspection.md) D1/D2). The eager depth walk crashed the engine (native `SIGSEGV` in coreclr's unwinder) over a wide live object graph. `drhook_locals` now returns ONE level with a `hasChildren` flag; the new `drhook_expand` tool navigates deeper on demand, one bounded level at a time — full observability without the eager walk. Extended to VALUETYPE structs/spans and BYREF ref-struct receivers. 119/119 DrHook engine tests.

### Added

- `drhook_expand` MCP tool — lazy, navigable, one-level-per-call expansion of an object / array / struct / span at a stop.
- `MaxAtomSize`-bounded growable result-binding buffer; `LongLiteralRoundTripTests`.
- ADRs: Mercury [ADR-050](docs/adrs/mercury/ADR-050-growable-result-binding-buffer.md) (Completed), DrHook [ADR-013](docs/adrs/drhook/ADR-013-value-type-refstruct-inspection.md) (Accepted, D1/D2), [ADR-014](docs/adrs/drhook/ADR-014-inspection-fault-containment.md) (Proposed — inspection-fault containment + value-type read safety, the next gap the dogfood surfaced).

## [1.8.4] - 2026-06-21

**Mercury convergence: one execution path, one expression evaluator.** See [docs/releases/1.8.4.md](docs/releases/1.8.4.md).

### Changed

- **One SPARQL `[110]` expression evaluator** ([ADR-049](docs/adrs/mercury/ADR-049-unified-expression-evaluator.md), divergence S2 closed). FILTER and BIND now both evaluate `FilterEvaluator.EvaluateToValue` — FILTER derives its boolean via `CoerceToBool(...)`, BIND binds the produced term and leaves the variable unbound on error, exactly as SPARQL §17/§18 frame it. FILTER gains arithmetic and the full value grammar; BIND gains `||` / `&&` / `!` and the boolean predicates. The 9 BIND / SELECT-expression / GROUP-BY-expression call sites and the 3 per-row `IncrementBnodeRowSeed` calls were routed accordingly; FILTER switched to `CoerceToBool(ValueOr())`.
- **`TreeJoinExecutor` is the sole SPARQL execution path** ([ADR-047](docs/adrs/mercury/ADR-047-default-path-cutover.md)) — first released here (landed on `main` after 1.8.3). All four default-path carve-outs (VALUES+triple, sub-query, FROM, SERVICE) route through the tree.
- **Cross-format canonicalizers converged** onto shared `RdfEscape` / `RdfIri` / `RdfNumeric` in `Mercury/Rdf` (divergence S1, commits `afd73f8`→`8152f17`) — JSON-LD, Turtle, TriG, RDF/XML, N-Triples, and N-Quads delegate escape decoding, relative-IRI resolution, and numeric/boolean xsd typing instead of each carrying its own.

### Added

- `FilterEvaluator.EvaluateToValue` — the single `[110] Expression → Value` evaluator (logical connectives, comparisons, `IN`, arithmetic, every built-in), with base-IRI resolution for `IRI()`/`URI()` and `@lang` / non-numeric-datatype preservation in literal parsing.
- Two demonstrated-gap conformance locks: `Filter_WithArithmetic_Evaluates` and `Bind_WithLogicalOr_Evaluates`, plus the frozen `Adr049FunctionReconciliationProbeTests` pinning the exact lexical output of every shared built-in.

### Removed

- **`BindExpressionEvaluator` (2,336 lines)** and the old FILTER boolean-grammar duplication (~165 lines). (ADR-047 also removed the ~14 K-line slot-operator execution engine.)

### Fixed

- **Date/time accessors extract components from the lexical form** (`YEAR`…`SECONDS`, `TZ`, `TIMEZONE`). The old `DateTime.TryParse` path converted a timezone offset to machine-local time, giving a wrong, non-deterministic component (e.g. `HOURS` of `…15:38:02-08:00`).
- **`STRDT` / `STRLANG` / `DATATYPE` conformance** — `STRDT` no longer double-brackets the datatype IRI and rejects a language-tagged first argument (RDF 1.1 `xsd:string`-is-simple handled); `STRLANG` rejects a language-tagged first argument; `DATATYPE` recovers the datatype from the preserved lexical form so `decimal` / `double` / `integer` stay distinct. `LANG` of a plain literal returns `""` (not unbound).
- **Latent FILTER hang** on a compound function argument (`COALESCE(1 + 1, 0)` — the argument parser parked on the operator and spun forever); and `IRI(x)` now compares equal to the literal `<x>`.
...
- **`drhook_detach`** (detach, leave the target running) and **`drhook_kill`** (forced termination — anomaly path) MCP tools ([ADR-011](docs/adrs/drhook/ADR-011-lifecycle-console-dashboard.md) D1), completing the `stop` / `detach` / `kill` lifecycle triad. The triad is now complete for both ownership models: `drhook_detach` leaves an Owned (launched) target running via `DetachLeaveRunning` with disown-and-reap (F-010-2, closed in 1.8.3) and detaches from Borrowed (attached) targets — refusing outstanding breakpoints (deactivate first). `drhook_kill` force-terminates Owned targets via `DebugSession.Abandon` (SIGTERM brief-grace → SIGKILL) and Borrowed (attached) targets (F-010-1, closed in 1.8.3). (`kill`, not `terminate`: "terminate" is the genus and collides with DAP's *graceful* terminate, while `stop` already owns graceful-for-Owned.) All three lifecycle tools require a `hypothesis` per Decision principle 5 — echoed in the response (for `kill`, the reason force was needed).
- **`drhook_drain_console`** and **`drhook_drain_log`** MCP tools ([ADR-011](docs/adrs/drhook/ADR-011-lifecycle-console-dashboard.md) D3) surface a launched debuggee's captured stdout/stderr and logpoint output to the agent — via a new surface-agnostic `IDebugEventSink.OnConsoleOutput` event and a `CompositeEventSink` fanning the anomaly / log / console channels to bounded buffers (the surface-agnostic seam ADR-012's future views consume — D5). Console / log / anomaly buffers reset at session start, so the `drhook_drain_*` tools reflect only the current session. **22 tools.**
- **`drhook_launch` honors a launch-time environment override** — closes the silent MCP→substrate drop where environment passed at the MCP layer never reached the spawned target (Owned-only, POSIX; probe 63 / finding 81).
- **Full pending-breakpoints + entry-module hold-gate** — breakpoints bind in lazily-loaded modules, and `drhook_launch` stops at a source breakpoint without requiring a `Debugger.Break` in the target.
...
- **Source breakpoints now bind when the source file name differs from the assembly name** ([ADR-011](docs/adrs/drhook/ADR-011-lifecycle-console-dashboard.md) D3 follow-up). `drhook_launch` / `drhook_attach` / `drhook_break_source` resolved the module by guessing it from the source-file stem (`Program.cs` → "Program"), so a breakpoint in a normal project (`Program.cs` compiled into `Launch.dll`) failed with "Could not set breakpoint." Resolution now walks the loaded modules and binds on the one whose Portable PDB actually references the file — a new `DebugSession.SetBreakpointAtLine(fileHint, line, policy)` overload; the stem heuristic is removed. Surfaced by the D3 live drain_log smoke; validated by probe 60 + the re-run (finding 76).
- **`drhook_continue` no longer NREs when the debuggee exits during the continue** ([ADR-011](docs/adrs/drhook/ADR-011-lifecycle-console-dashboard.md) D3 follow-up). A `suspend='none'` logpoint that runs to program end makes `WaitForStop` return `ProcessExited`; the response builder then walked a stale stop-thread (`CallbackPump` left `_stopThread` set on `ExitProcess`) → NRE. The pump now clears `_stopThread` on exit (so `GetStackFrames` returns empty for every stop-handler via `WalkManagedFrames`' existing `pThread == 0` guard), and `ContinueAsync` reports a clean `processExited` + cleans up the session — the drain buffers survive, so final logpoint/console output is still drainable — instead of a frame dump. Validated by probe 61 + the live re-run (finding 77).
- **`drhook_locals` no longer mis-serializes boxed values** — typed `JsonValue` dispatch for boxed `RawValue`, so `int` / `bool` / `double` / `string` render with their real types instead of via the object fallback.
- **Mercury: aggregation no longer dropped on the top-level GRAPH execution path** (`b1c5c18`). Solution modifiers are threaded through `FromMaterializedWithGraphContext` + the `QueryExecutor` call site, and the shared materialized-with-graph-context constructor sets `_isMaterialized` so the grouped-collection path scans materialized rows rather than an uninitialized scan. The shared constructor also closes the same latent subquery-aggregate case. Conformance-blind (the W3C suite exercises default-graph aggregation only); surfaced within minutes dogfooding semantic-memory recall, where all data lives in named graphs. +4 `QueryExecutorTests.GraphAggregation` tests (RED before, GREEN after); 421/421 SPARQL 1.1 Query conformance held.
...
**Headline:** [ADR-043](docs/adrs/mercury/ADR-043-metric-emission-decoupling.md) Metric Emission Decoupling shipped — bounded staleness for live observability under shared-disk pressure. Cycle 9 trigram drain's 2+ hour `rebuild.jsonl` staleness symptom (workload mmap I/O queue saturation starving small metric writes) is structurally bounded by a three-part fix: periodic explicit `FileStream.Flush()` on the JSONL writer (5s default), `MetricEmissionThrottle` helper applied at the merge emit site, and `ProgressEmissionMinInterval` default lowered 30s → 5s for the rebuild/drain emit sites (which already had time-based gating). Operator runbook for the separate-disk `--metrics-out` pattern published in `docs/operations/`. Additionally: `observability-discipline-systematic-not-reactive` limit Resolved at the 1.8.0 boundary — structural cure lives in James (the cognitive orchestrator); Martin mitigates manually until James is active in 1.8.x.

### Added

- **`src/Mercury/Diagnostics/MetricEmissionThrottle.cs`** — new static class for time-based emission throttling. `ShouldEmit()` returns true at most once per `minInterval`; atomic `Interlocked.CompareExchange` on the last-emit timestamp resolves concurrent callers to exactly one true per interval. `Reset()` forces the next call to return true regardless of interval (for end-of-phase emissions). 7 unit tests in `MetricEmissionThrottleTests`.
- **`docs/operations/runbook-metrics-disk-separation.md`** — new operator-facing runbook explaining when + how to route `--metrics-out` to a physically separate disk for runs > 1 hour. Covers macOS / Linux / Windows configuration + verification procedure.

### Changed

- **`src/Mercury/Diagnostics/JsonlMetricsListener.cs`** — added `_flushTimer` (mirrors existing `_stateTimer` pattern). Default 5s flush cadence, configurable via constructor `flushInterval` parameter (`TimeSpan.Zero` opts out). Calls `_writer.BaseStream.Flush()` under `_gate`. Does NOT call fsync — the OS page cache is "the file" from the perspective of `tail -f` and any in-flight reader.
- **`src/Mercury/Storage/SortedAtomStoreExternalBuilder.cs`** — `MergeAndWrite` paired the existing records-based gate (`recordsProcessed % MergeProgressEmissionInterval == 0`) with the new time-based `MetricEmissionThrottle` (5s default via `MergeProgressTimeInterval`). Emits on either gate, whichever fires first; throttle resets after each emit.
- **`src/Mercury/Storage/QuadStore.cs`** — `ProgressEmissionMinInterval` default lowered 30s → 5s. The rebuild + drain emit sites already used this for time-based gating; the lower default matches the merge-side throttle and bounds live JSONL tail-to-real-time staleness at the metric-emission layer.

### Resolved limits

- **`metric-emission-backpressure-on-shared-disk`** — surfacing trigger for ADR-043. Resolved by this release.
- **`observability-discipline-systematic-not-reactive`** — category error (reactive attention is fundamentally reactive). Resolved at the 1.8.0 boundary; structural cure lives in James (Sky Omega 2.0); Martin's working model in the interim is manual mitigation rather than relying on the agent's reactive attention.

### Validation

- Full Mercury test suite: **4,522 / 0 failed / 6 skipped** (was 4,515 baseline pre-ADR-043; +7 from `MetricEmissionThrottleTests`).
- W3C SPARQL 1.1 conformance: unchanged.
- No existing test relied on the 30s `ProgressEmissionMinInterval` default; tests that need specific timing already set the value explicitly.
