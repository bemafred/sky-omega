...
public partial class QueryExecutor
{
    ...
    /// <summary>
    /// Execute a GRAPH-only query with EXISTS/NOT EXISTS filters and return lightweight materialized results.
    /// This method evaluates EXISTS filters during materialization to avoid the ~22KB QueryResults struct.
    /// Use this for GRAPH queries that contain EXISTS or NOT EXISTS filters.
    /// Caller must hold read lock on store.
    /// </summary>
    [System.Runtime.CompilerServices.MethodImpl(System.Runtime.CompilerServices.MethodImplOptions.NoInlining)]
    internal MaterializedQueryResults ExecuteGraphWithExistsToMaterialized()
    {
        if (!_buffer.HasGraph || _buffer.TriplePatternCount != 0)
        {
            return MaterializedQueryResults.Empty();
        ...
        if (graphCount > 1)
        {
            graphResults = CollectAndJoinGraphResultsSlotBased(patterns);
        }
        else if (_buffer.FirstGraphIsVariable)
        {
            if (_buffer.FirstGraphPatternCount == 0)
        ...
        {
            graphResults = ExecuteFixedGraphSlotBasedList(patterns);
        }
        ...
        // Filter results by evaluating EXISTS/NOT EXISTS for each row
        var filteredResults = FilterResultsByExists(graphResults, graphContext);
        ...
        var bindings = new Binding[16];
        var stringBuffer = _stringBuffer;
        return new MaterializedQueryResults(filteredResults, bindings, stringBuffer,
            _buffer.Limit, _buffer.Offset, _buffer.SelectDistinct);
    }
