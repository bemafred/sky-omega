...
internal sealed class TriGStreamParser : IDisposable, IAsyncDisposable
{
    ...
    private ReadOnlySpan<char> ParseIriRef()
    {
        ...
        else
        {
            ...
            var resolved = RdfIri.Resolve(_baseUri, iriContentCopy);
    ...
    // IRI resolution (RFC 3986 §5.2) is shared with every RDF-family parser via Rdf/RdfIri.cs (docs/divergence
    // S1b) — the former bespoke ResolveIri / ParseUri / RemoveDotSegments cluster is deleted.
