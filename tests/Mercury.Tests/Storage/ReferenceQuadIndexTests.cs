...
public class ReferenceQuadIndexTests : IDisposable
{
    ...
    #region Sort-insert (ADR-030 Phase 3)
    ...
    public void AppendSorted_QueryResultsMatchRandomInsert()
    {
        ...
        {
            // Insert in arbitrary (non-sorted) order.
        ...
        {
            // Sort explicitly and AppendSorted.
        ...
        // Compare via full scans — both should have 300 quads and the same projection.
        ...
        // Predicate-bound scan should return identical counts.
        int randomCount = 0, sortedCount = 0;
        var re = randomReopened.Query(1, 5, -1, -1); // Primary=5 wildcard S/T
        ...
        Assert.Equal(10, sortedCount); // Primary=5 has 10 entries
    }

    [Fact]
    public void AppendSorted_DuplicateKey_IsSilentNoOp()
    {
        // ADR-029 Decision 7 uniqueness: exact (g, p, s, t) duplicates are no-ops.
        // Sort-insert's fast path handles this inline via the equality check.
        var idx = CreateIndex();
