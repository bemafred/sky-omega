...
| **DrHook.Mcp** | **972** | 17 stepping MCP tools (attach, launch, breakpoints + conditions, exception filters with subclass-aware filtering, locals + field/array inspection ≥ depth 2, step, continue, pause) + `drhook_processes` + `drhook_snapshot`, all backed by `DrHook.Engine.DebugSession`. `drhook_step_test` is wired but returns "not yet ported" — slated for removal in [ADR-007](docs/adrs/drhook/ADR-007-teardown-concurrency-test-debug.md) Phase 7. |
