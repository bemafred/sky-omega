...
|---------|------:|----------:|-------|
| Mercury.Tests | 61,903 | ~4,454 | Most recent: **+~38 since 1.7.64 in the 1.7.65 → 1.7.69 ADR-029 matrix-completion arc** — 17 Minimal-profile tests (8 storage-layer in `MinimalQuadIndexTests.cs` + 9 E2E in `QuadStoreMinimalProfileTests.cs`, 1.7.68) + BTreeFile-extraction regression coverage (1.7.69, magic-number-mismatch detection at construction). Prior +30 across 1.7.58 → 1.7.64: 4 ADR-041 cleanup tests (`BulkTmpCleanup_SuccessPath`, `BulkTmpCleanup_MergeException`, `BulkTmpCleanup_PreserveFlagSet`, `BulkTmpCleanup_NonBulkTmpPath_FailsAssertion`) + 3 ADR-042 tests (`Build_SpanApi_ProducesIdenticalResultToFuncApi` for Parts 1+4 equivalence, `Build_SpanApi_ScratchBufferReuseDoesNotCorruptHash` defensive scratch reuse, `Build_SinkApi_WritesSameEntriesAsChunkedArrayPath` for Part 2 sink equivalence) + 3 ADR-040 `ProcessMemoryProbe` smoke tests (positive value, plausible range, ≥ 1 MB sanity floor). Earlier in 1.7.48 → 1.7.50 arc: BoundedFileStreamPoolTests + cap-eviction validation + 1.7.49 cleanup hook tests + 3 ADR-037 pipelined-spill tests. |
