...
| ├─ Diagnostics | 3,009 | Observability infrastructure; ADR-035 Phase 7a metrics + ADR-039 MPHF instrumentation (1.7.56) + ADR-040/041/042 observability events (1.7.58 → 1.7.64): `bulk_tmp_cleanup`, `merge_readahead_budget`, `mphf_memory_budget` |
...
| ├─ NTriples | 1,350 | N-Triples parser and writer (1.7.59: AggressiveInlining on `Peek` + `PeekAhead` for the per-IRI hot path) |
| ├─ Owl | 566 | OWL/RDFS reasoner |
| ├─ Rdf | 536 | Core RDF types |
| └─ (top-level + obj) | 1,135 | `RdfEngine`, `SparqlEngine` facades + generated files |
...
| **DrHook.Engine** | **4,071** | Runtime observation substrate — BCL + P/Invoke + source-gen COM. ICorDebug via per-RID `libdbgshim` (Microsoft.Diagnostics.DbgShim.<rid> NuGet); EventPipe for processes/snapshot via Microsoft.Diagnostics.Tracing.TraceEvent. **Substrate-independence reached at 1.8.2** — `src/DrHook/` (netcoredbg-DAP-backed predecessor) retired entirely. 47 unit tests pass; 40 PoC probes (02–40) validate substrate against macOS/arm64 CoreCLR (39 PASS + 1 documented PARTIAL = probe 05 RESOLVED by probe 06). |
| **DrHook.Mcp** | **972** | 18 MCP tools — session lifecycle (`drhook_launch` / `drhook_attach` / `drhook_detach`), execution control (continue / pause / step over / into / out), breakpoints (source / function / exception with subclass-aware filtering + condition / hit-count / logpoint policy + list / remove / clear), `drhook_locals` (field/array inspection ≥ depth 2), plus `drhook_processes` / `drhook_snapshot` / `drhook_drain_anomalies` — all backed by `DrHook.Engine.DebugSession`. Tool names follow IDE-debugger convention per [ADR-010](docs/adrs/drhook/ADR-010-mcp-tool-surface-redesign.md) Tier 1 (renamed from the `drhook_step_*` family; `drhook_step_test` removed). |
...
| Minerva.Tests | — | — | |
| DrHook.Engine.Tests | 641 | 47 | Unit-only coverage at write time — CallbackPump (callback marshalling, mscordbi event-thread enqueue), ManagedCallbackHost (source-gen COM CCW v-table), ProcessInspector (EventPipe enumeration), SymbolReader (portable-PDB locals + line maps), BreakpointPolicy (hit-count gate admission, log-record defaults, policy defaults), BoundedLogSink (ring-buffer drain order + capacity bounds + thread-safety). End-to-end MCP-surface integration coverage is the open ADR-006 Validation gate; ADR-007 Phase 8 closes it via Phase 2's mechanism. |
