...
| **Mercury.Runtime** | **3,548** | Buffers, cross-process gate, temp paths; ADR-040/042 Part 5 `ProcessMemoryProbe` shared P/Invoke probe (1.7.63) |
...
| Benchmarks | ~3,362 |
| Examples | ~972 |
| Documentation | ~42,257 |
| **Grand total** | **~211,270** |

Net change from 1.7.50 to 1.7.64: +9,286 lines. Substrate growth dominated by the ADR-040/041/042 arc (+~700 lines across `Storage` + `Mercury.Abstractions` + `Mercury.Runtime`) + `Compression` parallel decompressor (+~800 lines) + ADR-038 + ADR-039 substrate work shipped in cycle 10 r4 (+~1,200 lines `Storage`) + Mercury validations & limits docs (+~2,000 lines under `docs/`).
