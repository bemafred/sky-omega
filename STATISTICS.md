...
| **DrHook.Engine** | **6,231** | Runtime observation substrate — BCL + P/Invoke + source-gen COM. ICorDebug via per-RID `libdbgshim` (Microsoft.Diagnostics.DbgShim.<rid> NuGet); EventPipe for processes/snapshot via Microsoft.Diagnostics.Tracing.TraceEvent. **Substrate-independence reached at 1.8.2** — `src/DrHook/` (netcoredbg-DAP-backed predecessor) retired entirely. **1.8.3** added the ADR-011 lifecycle/console substrate (`DetachLeaveRunning`, `posix_spawn` console isolation, `CompositeEventSink`, full pending-breakpoints). 89 `DrHook.Engine.Tests` test methods; PoC probes through 63 (81 findings + 172 fixtures) validate the substrate against macOS/arm64 CoreCLR. |
| **DrHook.Mcp** | **1,566** | 22 MCP tools — session lifecycle (`drhook_launch` / `drhook_attach` / `drhook_stop` / `drhook_detach` / `drhook_kill`), execution control (continue / pause / step over / into / out), breakpoints (source / function / exception with subclass-aware filtering + condition / hit-count / logpoint policy + list / remove / clear), `drhook_locals` (field/array inspection ≥ depth 2), plus `drhook_processes` / `drhook_snapshot` / `drhook_drain_anomalies` / `drhook_drain_console` / `drhook_drain_log` — all backed by `DrHook.Engine.DebugSession`. Tool names follow IDE-debugger convention per [ADR-010](docs/adrs/drhook/ADR-010-mcp-tool-surface-redesign.md) Tier 1 (renamed from the `drhook_step_*` family; `drhook_step_test` removed); every state-changing or state-reading tool takes a `hypothesis`. |
...
| Minerva.Tests | — | — | |
| DrHook.Engine.Tests | 1,378 | 89 | Unit-only coverage at write time — CallbackPump (callback marshalling, mscordbi event-thread enqueue), ManagedCallbackHost (source-gen COM CCW v-table), ProcessInspector (EventPipe enumeration), SymbolReader (portable-PDB locals + line maps), BreakpointPolicy (hit-count gate admission, log-record defaults, policy defaults), BoundedLogSink (ring-buffer drain order + capacity bounds + thread-safety). End-to-end MCP-surface integration coverage is the open ADR-006 Validation gate; ADR-007 Phase 8 closes it via Phase 2's mechanism. |
